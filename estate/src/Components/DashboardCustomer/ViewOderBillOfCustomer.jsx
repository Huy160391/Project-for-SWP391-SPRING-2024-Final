        ...
        const fetchOrderDetails = async () => {
            ...
            try {
                ...
                const orderDetailsWithExtraInfo = await Promise.all(orders.map(async (order) => {
                    ...
                    const agencyResponse = await axios.get(`https://localhost:7137/api/Agencies/${order.agencyId}`);
            ...
            {orderDetails.map((detail, index) => (
                <div key={index} className="flex flex-col md:flex-row bg-white border-t shadow-xl rounded-lg overflow-hidden mb-8">
                    ...
                    <div className="p-4 flex-grow">
                        ...
                        <p className="text-gray-600"><span className="font-medium">Agency Name:</span> {detail.agencyName}</p>
                        <p className="text-gray-600"><span className="font-medium">Agency Address:</span> {detail.agencyAddress}</p>
                        <p className="text-gray-600 mb-4"><span className="font-medium">Agency Phone:</span> {detail.agencyPhone}</p>
