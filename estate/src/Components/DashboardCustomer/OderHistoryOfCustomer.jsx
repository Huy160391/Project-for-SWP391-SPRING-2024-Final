...
const OderHistoryOfCustomer = () => {
  ...
  if (allOrders.length === 0) {
    return <div className="text-center font-bold font-serif">You don't have any orders yet</div>;
  }
  return (
    <div className="flex flex-col w-full font-serif">
      ...
      <div className="mt-5 ml-5 mb-6 flex justify-start items-center space-x-10">
        <div>
          <label htmlFor="building-select" className="block text font-medium text-gray-700">Building Name:</label>
          <select id="building-select" value={selectedBuilding} onChange={handleBuildingChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text rounded-md shadow">
        ...
        <div>
          <label htmlFor="agency-select" className="block text font-medium text-gray-700">Agency Name:</label>
          <select id="agency-select" value={selectedAgency} onChange={handleAgencyChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text rounded-md shadow">
            <option value="">All</option>
        ...
        <div>
          <label htmlFor="status-select" className="block text font-medium text-gray-700">Status:</label>
          <select id="status-select" value={selectedStatus} onChange={handleStatusChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text rounded-md shadow">
            <option value="">All</option>
        ...
        <button onClick={handleSearch} className="mt-4 inline-flex justify-center py-3 px-5 border border-transparent shadow text font-medium rounded-md text-white bg-blue-500 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow">
          Search
        </button>
        <button
          type="button"
          onClick={() => navigate(-1)}
          className="mt-4 inline-flex justify-center py-3 px-5 border border-transparent shadow text font-medium rounded-md text-white bg-blue-500 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow">
          Back
        </button>
      </div>
      {orders.map((order, index) => (
        <div key={index} className="bg-white shadow overflow-hidden sm:rounded-lg mb-5 font-serif">
          <div className="px-4 py-5 sm:px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-white  rounded-lg">
            ...
            </div><div className="text font-medium text-gray-700 ">
          ...
          <div className="px-4 py-4 sm:px-6">
            ...
            {order.status === "Unpaid" && (
              <div>
                ...
                <button
                  ...
                  className="mb-5 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 float-right" // Thêm lớp CSS float-right để căn nút về bên phải
                >
            ...
            {order.status === "Complete" && (
              <div>
                <h1 className="text-4xl font-bold text-green-200 px-3 py-2 w-52 bg-green-500 rounded-lg ml-20">
                  Complete
                </h1>
            ...
            {pageNumbers.map(number => (
              <li key={number} className="mx-1">
                <button
                  ...
                  className={`px-4 py-2 text font-semibold leading-5 transition-colors duration-150 border border-gray-300 rounded-md focus:outline-none focus:shadow-outline-blue ${currentPage === number ? 'bg-blue-500 text-white border-blue-600' : 'text-blue-700 bg-white hover:bg-blue-100 hover:text-blue-700'}`}
                >
                  {number}
                </button>
              </li>
