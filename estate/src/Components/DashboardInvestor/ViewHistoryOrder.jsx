...
const ViewHistoryOrder = () => {
    ...
    const fetchOrders = async () => {
        try {
            ...
            const filteredOrders = orderRes.data.filter(order => order.status === "Complete");
    ...
    const handleSearch = () => {
        ...
        if (selectedBuilding) {
            filteredOrders = filteredOrders.filter(order => order.buildingName.toLowerCase() === selectedBuilding.toLowerCase());
        }
    
        if (selectedAgency) {
            filteredOrders = filteredOrders.filter(order => order.agencyName.toLowerCase() === selectedAgency.toLowerCase());
        }
    
        if (selectedCustomer) {
            filteredOrders = filteredOrders.filter(order => order.customerName.toLowerCase() === selectedCustomer.toLowerCase());
        }
    ...
    return (
        <div className="flex flex-col w-full border-8 rounded-lg border-gray-200">
            ...
            <div className="mt-5 ml-5 mb-6 flex justify-start items-center space-x-10">
                ...
                <button onClick={() => navigate(-1)} className="mt-4 inline-flex justify-center py-3 px-5 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-500 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow">
                    Back
                </button>
            </div>
                    ...
                    <div className=''>
                        <div className="px-4 py-4 sm:px-6">
                            <div className=" text-2xl font-medium text-gray-700 text-right mb-3 ">
                                <strong>Total Amount:</strong> <span className="text-green-600 text-4xl">${order.apartmentPrice}</span>
                            </div>
